...
const PortfolioScreen: React.FC = () => {
    ...
    const RenderCategories = (category: any, key: number) => {
        if (!category) return ""

        return key === 0 ? <span className="mr-5">{category.name}</span> : <span> & {category.name}</span>
    }
                                                    ...
                                                    <div className="bd-portfolio-text">
                                                        ...
                                                        <div className="d-flex align-items-center">
                                                            {project.category?.map((category: any, key: number) => (
                                                                <span key={`all-category-${key}`} className="mb-2 capitalize">
                                                                    {RenderCategories(category, key)}
                                                                </span>
                                                            ))}
                                                        </div>
